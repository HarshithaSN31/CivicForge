...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    async function checkAuthSession() {
      try {
        ...
        if (err.name === 'UserUnauthenticatedException' || err.message?.includes('not authenticated')) {
          ...
          setAuthError('AWS Cognito authentication service is temporarily unavailable. (AWS Cognito deployment required: npx ampx sandbox)');
        }
      } finally {
        setIsLoading(false);
      }
  ...
  const switchRole = (role: UserRole) => {
    if (role === 'AUTHORITY' && currentUser?.role !== 'AUTHORITY') {
      alert('🔒 Access Denied: Municipal Authority portal requires an authenticated Authority Cognito account.');
      return;
    }
  ...
  const registerUser = async (input: SignUpInput) => {
    try {
      ...
      if (isUnconfiguredError(err)) {
        throw new Error('AWS Cognito authentication service is temporarily unavailable. Please deploy your backend resources using `npx ampx sandbox` with valid AWS credentials.');
      }
