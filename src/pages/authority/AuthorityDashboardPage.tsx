...
export const AuthorityDashboardPage: React.FC = () => {
  ...
  const centerCoordinates: [number, number] = incidents.length > 0
    ? [incidents[0].primaryLocation.latitude, incidents[0].primaryLocation.longitude]
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-civic-navy text-white p-6 rounded-xl shadow-civic">
        <div>
          <span className="text-xs font-bold text-amber-400 uppercase tracking-wider flex items-center gap-1">
            <LayoutDashboard className="w-4 h-4" /> Operations Overview (India)
          </span>
          <h1 className="text-2xl font-extrabold tracking-tight mt-1">Civic Intelligence Command Dashboard</h1>
          <p className="text-xs text-slate-300 mt-0.5">
            Real-time incident triage, automated report clustering, and multi-department dispatch across Indian municipalities.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card>
          <CardBody className="p-4 flex items-center justify-between">
            <div>
              <p className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Resolved Recently</p>
              <h3 className="text-2xl font-extrabold text-emerald-600 mt-0.5">{resolvedThisWeek}</h3>
              <p className="text-[10px] text-emerald-700 font-semibold mt-1">Real database resolutions</p>
            </div>
            ...
            <CardBody className="p-0 divide-y divide-slate-100">
              {incidents.length === 0 ? (
                <div className="p-8 text-center text-slate-500 text-xs">
                  No civic incidents have been reported yet.
                </div>
              ) : (
